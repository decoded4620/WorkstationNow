...
if($includes_console_feedback_ps1 -ne $true)
{
    ...
    function Get-LogLevel-String([int]$Level){
        ...
        switch($Level)
        {
            {$_ -eq $LogLevel.Error}{ $retVal = "ERROR"     }
            {$_ -eq $LogLevel.Warn} { $retVal = "WARN"      }
            {$_ -eq $LogLevel.Fail} { $retVal = "FAIL"      }
            {$_ -eq $LogLevel.Verbose -or $_ -eq $LogLevel.Debug}{ $retVal = "" }
            {$_ -eq $LogLevel.Pending}  { $retVal = "PENDING"   }
            {$_ -eq $LogLevel.Success}  { $retVal = "SUCCESS"   }
            {$_ -eq $LogLevel.Info}     { $retVal = "INFO"      }
            default                     {  $retVal = "INFO" }
        }
